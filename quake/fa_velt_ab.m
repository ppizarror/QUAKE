function ft = fa_velt_ab(rho, Vs, D, H)
% FT_VELT_AB: Calcula factor de amplificacion caso viscoelastico entre el
% afloramiento rocoso (a) y el basamento rocoso (b). H_RS,RB = 2M/(M+N).
%
% Parametros:
...
ft = @(w) fa_velt_ab_w(rho, Vs, D, H, 1, n, 2*pi*w);

end

function a = fa_velt_ab_w(rho, Vs, D, H, E1, n, w)
...
a = 2 * abs(E1) * F(n) / (abs(E(n)) + abs(F(n)));
